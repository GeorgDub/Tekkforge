...
function partsFuer(intensitaet, thema, kickFigur, jamMelos) {
  ...
  if (i >= 5) steps[1] = baue((s) => (imTakt(s) === 8 ? hit([60], 94, 22) : null));
  if (i >= 3 || kickFigur === "roll")
    steps[2] = baue((s) => {
      if (kickFigur === "roll" && takt(s) === 3) return hit([60], 98, 10);
      if (i >= 3 && (imTakt(s) === 4 || imTakt(s) === 12)) return hit([60], 104, 28);
      return null;
    });
  ...
  else if (i >= 4) steps[3] = baue((s) => (imTakt(s) === 12 ? hit([60], 94, 22) : null));
  if (i >= 1) steps[4] = baue((s) => (s % 4 === 2 ? hit([60], 80, 12) : null));
  if (i >= 3)
    steps[5] = baue((s) =>
      imTakt(s) === 14 || (i >= 4 && imTakt(s) === 6) ? hit([60], 86, 34) : null,
    );
  if (i >= 4) steps[6] = baue((s) => (s % 8 === 5 ? hit([60], 76, 13) : null));
  if (i >= 5) steps[7] = baue((s) => (s % 8 === 7 ? hit([60], 70, 11) : null));
  // Runder Bass: Offbeats, ab 4 ein 16tel-Pickup auf den Wechsel.
  if (i >= 2) {
    steps[8] = baue((s) => {
      if (s % 4 === 2) return hit([t.bass[takt(s)]], 106, 18);
      if (i >= 4 && imTakt(s) === 15) return hit([t.bass[(takt(s) + 1) % 4]], 96, 9);
      return null;
    });
  }
  // Melos: Traeger auf der Eins, Antwort auf der Drei jeden zweiten Takt,
  // Stab-Akzent, Laeufer ab 5 — alles einstimmig nahe Unity.
  if (i >= 2) steps[10] = baue((s) => (imTakt(s) === 0 ? hit([t.akkorde[takt(s)][0]], 90, 96) : null));
  if (i >= 3)
    steps[11] = baue((s) =>
  ...
  if (i >= 5) steps[13] = baue((s) => (imTakt(s) === 0 ? hit([t.akkorde[takt(s)][0]], 84, 96) : null));
  // Flaeche traegt ab Intensitaet 4 mit — Sonnendeck eben.
  if (i >= 4 || breakStelle)
    steps[14] = baue((s) => (imTakt(s) === 0 ? hit(t.akkorde[takt(s)], breakStelle ? 76 : 66, 96) : null));
