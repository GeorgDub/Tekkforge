...
const abschnitte = [
  ...
  },
  {
    ...
    punkte: [
      ...
      "<b>Vocal-Paare</b> — je Vocal-Paar zwei Patterns, die aufeinander zeigen: A trägt die erste Hälfte auf Part 16, B die zweite, und das Acht-Takt-Vocal läuft so lange, wie man will. Nach jedem Paar ein KICK-Pattern ohne Kette zum freien Weiterspielen. Kein Alternate mehr — Melodie nur auf Part 13, Vocal nur auf Part 16; gespielt wird durch Entmuten und Pattern-Wechsel am Gerät.",
      "<b>Groove, Variation, Motion</b> — das Timing des Originals wird auf dem Drums-Stem gemessen und als Swing auf jedes Pattern gelegt, die Groove-Vorlage liegt dem Set bei. Jedes Pattern der Kette bekommt eine eigene Handschrift: gestreute Anschläge, rotierende Hi-Hat, Ghost-Kick, alle vier Patterns ein Snare-Fill — der Drop bleibt der unveränderte Anker, Melodie und Vocals bleiben ganz. Dazu Motion-Sequenzen: der Filter geht über den Aufbau auf, im Drop fährt der Master-FX hoch und die Kick fällt im letzten Takt. Welche Parameter-Kennungen das Gerät dafür wirklich nutzt, ist aus der Werksbank abgeleitet und mit einem Testpattern am Ohr zu prüfen.",
      "<b>Speicher nach Gehör</b> — jeder Slot wird auf seinen Rolloff gemessen: liegt die Energie zu 95 % unter 9 kHz, bekommt er die halbe Abtastrate und kostet den halben Speicher, ohne hörbaren Verlust. Bass, Sub, Kicks und dunkle Vocals fallen darunter; Hi-Hats, Snare und Clap bleiben immer voll. Das Budget rechnet jetzt in Bytes wie das Gerät, und ein Wächter halbiert bei Überlauf erst Vocals, dann FX, dann Bass, bevor er etwas weglässt.",
      "<b>Schleifen, die rund laufen</b> — jede Schleife bekommt Loop-Punkte auf der Taktgrenze und 64 Slice-Marker, damit das Gerät sie selbst choppen kann, ohne dass die Melodie in Dateien zerschnitten wird. Wiederholt sich ein Vier-Takter nach der Hälfte, wird nur die Hälfte gespeichert und der Loop füllt den Rest — außer bei Vocals.",
      "<b>Die Bassline des Originals</b> — aus dem Bass-Stem liest TekkForge je Viertel den Grundton und legt die Noten auf den Bass-Part: die Tekk-Figur bleibt, die Tonhöhe folgt dem Lied. So spielt der Synth-Bass des Geräts die Linie des Originals — mit jedem der 362 Oszillatoren, nicht nur mit einem Sample.",
      "<b>Beat-Raster, Hook, Time-Stretch</b> — ein Beat-Tracker folgt dem Lied statt ihm ein Lineal aufzulegen, die 8-Takt-Fenster beginnen auf echten Downbeats. Der Hook — das Fenster, das am öftesten wiederkehrt — wird zum Drop. Und Schleifen, die zu weit vom Bank-Tempo entfernt liegen, werden gedehnt statt verstimmt: taktgenau in Originaltonhöhe, wo sie vorher zum asynchronen One-Shot wurden.",
  ...
  },
  {
    ...
    punkte: [
      ...
      "<b>Drums erkennen</b> — Anschläge werden gefunden und nach Klangfarbe in Kick, Snare, Clap, geschlossene und offene Hat eingeteilt; jede Klasse landet als eigene Spur auf dem passenden Drum-Part, mit Velocity aus der Anschlagstärke.",
  ...
  },
  {
    ...
    punkte: [
      ...
      "<b>Omnitribe fragen</b> — läuft auf dem Gerät die Omnitribe-Erweiterung (eine Firmware-Ergänzung aus einem Schwesterprojekt, die neben der gewohnten Firmware mitläuft), meldet sie sich auf Knopfdruck mit Version und einer Selbstdiagnose in fünf Stufen; dann lassen sich alle sechzehn Klangparameter je Part direkt setzen und zurücklesen — Tonhöhe, Filter, Resonanz, Lautstärke, Panorama, Stimmenmodus, Hüllkurve, Modulation, Glide, Effekt-Schalter — und der Sequencer per Play, Stopp und Position steuern. Jeder Regler zeigt ehrlich, ob Omnitribe diesen Weg am Gerät schon gehört hat oder ob er nur aus Tabellen hergeleitet ist. Ohne diese Erweiterung sagt die App das klar — und sonst nichts. Seit Kurzem hat die Erweiterung einen Modul-Lader: Auf Knopfdruck schickt die App einen Modul-Kopf — wahlweise ein Kunstprodukt für jeden Fehlerfall oder den echten Kopf jedes der zwanzig kompilierten Module —, den die Erweiterung prüft und quittiert. Das wurde am 17. September 2026 aus TekkForge heraus am Gerät bewiesen: alle zwanzig Köpfe gehen durch, die Grenze sperrt darüber. Eine zweite Stufe kann sogar ein ganzes Modul in freien Speicher laden und seine internen Adressen zurechtrücken — auch das am Gerät nachgemessen. Nur das tatsächliche Ausführen eines Moduls gelingt noch nicht: die Module brauchen eine Laufzeit-Umgebung, die die Erweiterung noch nicht mitbringt (ein Testlauf ließ die CPU hängen, ein Neustart stellte alles her), darum bleibt die Ausführung hinter einer Sicherung aus. Ebenfalls am Abend geklärt: die Tonhöhe lässt sich über diesen Weg zwar setzen, aber nicht hörbar modulieren — eine Eigenheit der Firmware, die die App nun ehrlich so anzeigt.",
    ],
...
<section class="seite">
  ...
  <h2>288 fertige Effekt-Presets zum Ausprobieren</h2>
  <p class="lead">Man muss nicht bei null anfangen: TekkForge bringt acht fertig eingestellte Preset-Sets mit — vier für die Effekte einzelner Parts, vier für den Gesamtklang. Zusammen decken sie fast jeden Effekt-Algorithmus ab, den die Hacktribe-Firmware kennt. Die beiden neuesten Sets („Bewegung“ und „Tekk-Modulation“) bringen alles in den Takt — Ringmodulator, Tremolo, Slicer, Wobble-Filter, LFO-Bitcrusher — und stützen sich dabei auf die dekodierten Werks-Presets des Geräts statt auf Vermutungen.</p>
  <ul>
    ...
    <li>Die beiden jüngsten Sets (September 2026) füllen die Lücken: <b>„Ketten“</b> bringt die neun Zwei-Effekt-Paare, die es noch nicht gab — Doppelfilter, Doppelzerre, Filter vor Kompressor statt dahinter —, weil die Reihenfolge zweier Effekte hörbar ist; <b>„Charakter“</b> gibt zwölf Master-Algorithmen je einen eigenen Klang, von der kurzen Tekk-Halle über Röhren-Crunch und Dub-Bandecho bis zur gedrückten Vinylplatte.</li>
    <li>Zu jedem der 96 Grund-Presets gehören <b>zwei Abwandlungen</b>: derselbe Effekt, in genau eine Richtung verschoben. Nacheinander in denselben Platz geschrieben, hört man den Unterschied — und sonst nichts.</li>
  ...
  <h3 class="unter-h">Vor der Firmware: der Bootloader</h3>
  <p>Vor der eigentlichen Firmware liegt im Flash des Geräts ein kleiner Starter, der Speicher und Takt einrichtet und die Firmware lädt. Ein Entwickler aus der Hacktribe-Gemeinschaft hat dafür einen eigenen gebaut: Er zeigt beim Einschalten ein Menü, startet die Firmware aus dem Flash oder lädt eine über USB in den Arbeitsspeicher, ohne etwas zu flashen — der Wechsel zwischen Synth- und Sampler-Firmware wird damit zur Sache von Sekunden —, sichert den ganzen Flash auf die Karte und hängt einen Debugger an die USB-Buchse. TekkForge verpackt diesen Starter so, wie das Gerät ihn über seine gewöhnliche Update-Funktion annimmt: Aus dem Programm entsteht der 128-KiB-Startblock samt Bootskript und Prüfsumme, daraus eine <b>BOOT.VSB</b> — und die Werkbank prüft jede Update-Datei vorher genau so, wie die Firmware sie prüft (Magic, Name, Identität, Länge, Revision), für beide Gerätevarianten getrennt. Einen Flash-Sicherungsabzug des Bootloaders kartiert sie in seine Regionen und macht aus jeder wieder eine Update-Datei — der Rückweg zur Werksfirmware liegt damit als Datei vor. Die Regeln stammen aus der Firmware selbst, zurückgelesen aus dem Maschinencode; geschrieben wird hier nichts ins Gerät, denn ein falscher Startblock ist nur noch mit Lötkolben und Debug-Stecker zu heilen. Und bevor man überhaupt daran denkt, holt die Werkbank den Startblock, der <b>jetzt</b> im Gerät steht, direkt aus dem Flash (über Hacktribes Lesekommando) und legt ihn als Datei ab — der Rückweg liegt damit bereit, bevor der erste Schritt getan ist. Auf demselben Weg sichert sie in gut zwei Minuten den <b>gesamten Flash</b> des Geräts — Bootloader, Firmware, Nutzerdaten, Pattern, Samples, Slices — als eine Datei, die vollständige Gerätesicherung, für die es bisher einen Debug-Stecker oder den Custom-Bootloader brauchte. Dabei kam heraus, wie Korg seinen Starter tatsächlich ablegt: in drei Stücken (Sprungtabelle, Programm, Daten), ohne Prüfsumme — und die Werkbank erkennt beides, Werkszustand wie Eigenbau. Und weil im Flash die 250 Patterns genau so liegen, wie das Gerät sie auf die Karte exportiert, schneidet die Werkbank aus der Sicherung mit einem Klick die komplette <b>Pattern-Bank</b> als Datei heraus — am Gerät belegt: 250 Patterns mit ihren Namen, in TekkForge sofort ladbar. Und wer nur die Patterns will, holt sie in einer halben Minute direkt aus dem Gerät — die Pattern-Sicherung braucht damit weder SD-Karte noch den ganzen Abzug. Genauso einzeln kommen die Firmware, die Samples oder die Slice-Daten aus dem Gerät — jeweils fertig als Update-Datei mit dem Kopf, den das Gerät erwartet, und vorher geprüft; am Gerät belegt: die Firmware in 13 Sekunden, die 8 MiB Samples in 50. Auch die Global-Einstellungen, wie das Gerät sie im Flash abgelegt hat, liest die Werkbank benannt aus — MIDI-Kanal, Clock-Quelle, Chain Mode, Batterietyp und fünfzehn weitere — und hält sie gegen die Werks-Vorgabe, die daneben liegt; dabei fand sich auch, wo der Werksreset seine Patterns hernimmt: aus einem komprimierten Abbild der Werksbank, gleich hinter der Werks-Vorgabe. Dessen Kompressionsverfahren — Korgs eigenes, verwandt mit dem, was ZIP benutzt — ist aus dem Maschinencode nachgebaut, und die Prüfsumme am Ende stimmt: Die Werkbank holt damit in gut einer Sekunde den <b>Werkszustand aller 250 Patterns</b> als Datei aus dem Gerät, ohne einen Werksreset auszulösen, und sagt dazu, welche Patterns im Gerät seitdem verändert wurden — am Gerät waren es sieben. Auch die Slice-Daten der Samples, die das Gerät in einer eigenen Flash-Region führt, liest die Karte als das, was sie sind: dieselben Slice-Tabellen, die in jeder exportierten Sample-Datei stecken — im Gerät liegen noch die Werks-Slices von 32 Samples. Und wer morgens wissen will, wie es dem Gerät geht, drückt einen Knopf: In einer halben Minute entsteht der <b>Gerätebericht</b> — Kennungen, Einstellungen, welcher Bootloader im Gerät steht, welche Patterns vom Werk abweichen, Slices, laufende Stimmen und die ganze geladene Sample-Bank — als Textdatei, die Pattern-Bank gleich daneben. Und für den Weg zurück baut die Werkbank ein fertiges <b>SD-Update-Paket</b>: die laufende Firmware aus dem Gerät gelesen, eine gewählte Klangdatei dazu, beide geprüft und in die Ordnerstruktur gelegt, die das Gerät beim Update erwartet — mit einer Notiz, die Länge und Prüfsumme jeder Datei nennt. Der Nutzer kopiert den Ordner auf die Karte und spielt am Gerät ein; nichts davon berührt das Gerät von der App aus.</p>
  <p>Der Starter selbst hatte einen Haken, den erst der Blick in die Bytes zeigte: Er bog an einer festen Adresse einen Sprung um, damit die Synth-Firmware auf Sampler-Hardware durch ihr Herkunftstor kommt — im Sampler-Programm liegt an derselben Stelle aber der Rücksprung einer Funktion, die beim Stoppen des Sequencers läuft, und die wäre damit zerstört gewesen. Der eigene Umbau erkennt am Bild im Speicher, welche Firmware-Familie gerade geladen ist, und rührt das Sampler-Programm nicht mehr an; außerdem lädt er Update-Dateien von der Karte jetzt am richtigen Versatz. Und er kann, was bisher nur Korgs Update-Funktion konnte: jede Update-Datei (Firmware, Startblock, Samples, Nutzerdaten, Slices) aus seinem Datei-Browser heraus in ihren Flash-Bereich schreiben — bei der Firmware mit der Wahl <b>Boot</b> (nur in den Arbeitsspeicher, nach dem Ausschalten spurlos) oder <b>Flash</b>; vorher prüft er Magic, Name, Identität und Länge wie das Original, liest die ganze Datei erst in den Speicher und schreibt dann in einem Zug mit Rücklesen. Von TekkForge aus gibt es dazu vier Wege, alle hinter einer getippten Bestätigung und keiner schreibt Flash: den Starter selbst <b>über MIDI flüchtig starten</b> (Hacktribes Ladeweg nachgebaut, mit Handschlag, bevor ein Byte fließt), eine Firmware <b>per USB direkt in den laufenden Starter laden</b> (kein Kartenwechsel mehr), einen <b>Karten-Ordner vorbereiten</b>, der jede Datei benennt und sagt, was nicht auf die Karte gehört, und eine <b>SysEx-Datei senden</b>, etwa das Modul-Bündel der Omnitribe-Erweiterung. Alles kompiliert, im Programmbild geprüft und mit Attrappen getestet — am Gerät noch nicht gebootet; genau dafür ist der flüchtige Weg da.</p>
  ${img("boot-panel", "Der Startblock, gelesen aus dem angeschlossenen Gerät: Korgs Werks-Starter in drei Sektionen, sofort als Rückweg-Datei gesichert.")}
  <h3 class="unter-h">Die Sample-Liste ist eine Tabelle</h3>
  <p>Was das Gerät unter Sample 001 bis 274 zeigt — SAW, PULSE, die Noise-Modelle, dann über hundert FM- und VPM-Varianten —, sind keine Aufnahmen, sondern Einträge einer Tabelle in der Firmware: ein Name, ein Klangprogramm des Signalprozessors und ein paar Parameter. Die Serienfirmware führt in derselben Tabelle die Namen ihrer Werks-Samples; Hacktribe hat sie durch seine Varianten ersetzt und 147 Plätze frei gelassen. Zwei Zellen im Code sagen dem Gerät, wie lang die Liste ist — derselbe Mechanismus wie beim Effekt-Menü, das TekkForge schon am Gerät verlängert hat. Die Werkbank hängt deshalb eigene Varianten an: eine Vorlage wählen, Name, Verstimmung oder Ratio und Pegel setzen, anhängen — oder für ein FM-Modell gleich die ganze Reihe der Halbtöne, die Hacktribe ausgelassen hat. Die Verstimmung folgt dabei Hacktribes eigener, nicht linearer Kennlinie, aus dessen Einträgen abgelesen; Zwischenwerte sind geschätzt und als solche markiert. Zum Probieren ohne Flashen geht das flüchtig in den Arbeitsspeicher des Geräts. Dabei zeigte sich, dass das Gerät beim Einschalten eine Kopie der Tabelle anlegt und nur diese anzeigt — ein Eintrag, der allein in der Firmware-Tabelle steht, bleibt unsichtbar; die Werkbank schreibt deshalb flüchtig in die Kopie, die Firmware baut sie beim Start selbst. Am Gerät bestätigt: Platz 275 „X-SAW -3“ erschien in der Liste und klang richtig. Die erste so gebaute Firmware trägt 88 neue FM-Varianten, je 22 für X-SAW, X-SQUARE, X-TRI und X-SINE — eingebrannt, und das Gerät zeigt die Plätze 275 bis 362. Auf Wunsch sortiert der Bau die Liste anschließend ein: je FM-Modell alle 49 Halbtöne in einer Reihe, die VPM-Modelle dahinter — so, wie man sie am Gerät durchdreht. Die Gesamtfirmware vereint das mit den erweiterten Effekt-Presets, den Groove-Vorlagen und dem eigenen Startbild.</p>
  <p>Denselben Weg geht die Werkbank bei den <b>Modulationstypen</b>: auch sie sind eine Tabelle in der Firmware, in der Wellenform, Taktsynchronisation und Ziel getrennte Bytes sind. Daraus entstehen 36 Kombinationen, die es bisher nicht gab — freilaufende Sägezahn-, Rechteck- und Sample&amp;Hold-Modulationen und ein Zufall im Takt, jeweils auf Filter, Tonhöhe, Oszillator, Pegel, Pan oder Effekt. Sie lassen sich flüchtig ins Gerät legen oder einbrennen. Dabei fiel die eigentliche Hürde: die Firmware klemmt jeden Modulationstyp über 72 an 25 Stellen im Code — Lader, Setter, Eintragszeiger, ein Reglerfeld je Part — und setzt ihn beim Laden auf 1 zurück, Hacktribes Sinus-Typen eingeschlossen. TekkForge zieht alle 25 Stellen in einem Zug nach und verlegt das Feld in freien Speicher; ob das Menü danach über 96 hinausdreht, entscheidet die Probe am Gerät.</p>
  <p>Ein Knopf <b>Gerät ↔ Basis</b> liest den tatsächlichen Stand aus dem laufenden Gerät — Oszillator-Liste, Grenze im Code, Modulationstabelle — und hält ihn gegen die geladene Firmware-Datei. Was flüchtig geschrieben wurde und was schon eingebrannt ist, steht damit schwarz auf weiß, ohne aufs Display zu schauen.</p>
  <div class="hinweis"><b>Nebenbei repariert:</b> Der Weg von der Datei aufs Gerät war bisher versperrt — die Pflicht-Lesung des Ziel-Platzes warf das geladene Preset wieder aus dem Editor. Jetzt bleibt es stehen, und die Lesung holt nur noch das, wofür sie da ist: den Vorher-Stand für den Rückweg. Jede der 288 Dateien wird zudem automatisch geprüft — Größe, Algorithmus, und dass jede Regler-Zuordnung auf einen Parameter zeigt, den es wirklich gibt.</div>
</section>
...
<section class="seite">
  ...
  <h3 class="unter-h">Der Geräte-Monitor — Zahlen mit Namen</h3>
  <p>Aus einer vollständigen Analyse der Sampler-Firmware (Tausende benannte Funktionen, von einem Mitstreiter mit Ghidra erarbeitet und in TekkForge übernommen) kennt die App inzwischen die wichtigsten Zustandsspeicher beim Namen: die 24 Stimmen-Plätze, die der Prozessor an den Klangchip vergibt — welcher Part spielt gerade welchen Oszillator mit welcher Note —, den Zähler der Noten-Ereignisse, die Batteriestufe mit ihren Schwellen und den Katalog der geladenen User-Samples. Ein Klick liest das aus dem laufenden Gerät und zeigt es lesbar an. Am Gerät belegt: Eine angespielte Note zählt den Ereigniszähler um zwei hoch (An und Aus), und die Batteriestufe verrät vorab, ob das Gerät ein SD-Update überhaupt zulassen würde.</p>
  ${img("geraete-monitor", "Der Geräte-Monitor im Speicherzugriff: Stimmen-Plätze, Ereigniszähler und Batteriestufe, gelesen aus dem laufenden Gerät.")}
