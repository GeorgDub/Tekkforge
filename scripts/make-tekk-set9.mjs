/**
 * Erzeugt TEKK_SET9.e2sallpat — "LANGSTRECKE": 100 Patterns, 175 BPM, fuer
 * tekk4.all. Das erste Langformat: zehn Segmente mit SECHS rotierenden
 * Melo-Themen (inkl. der neuen Acid- und Vocal-Farben aus tekk4), jedes
 * Segment endet in einem loopenden Jam seines Themas.
 *
 * Der Plan wird programmatisch aus einem Segment-Template erzeugt — bei 100
 * Patterns waere eine Handliste nur Tippfehlerfutter. Alle Lektionen an
 * Bord: Mute-Zock-Fuellung, Kick-IFX 09 LOW EQ, MFX 12 GRAIN SHIFTER,
 * echter Bass, einstimmige Melos nahe Unity, Headroom, Jams mit
 * Starter-Steps.
 */
...
const ZIEL = process.argv[2] ?? "examples/e2s/TEKK_SET9.e2sallpat";
const N = 64;
const BPM = Number(process.argv[4]) || 175;
...
const THEMEN_FOLGE = ["T1", "T2", "T3", "T4", "T5", "T6", "T2", "T5", "T3", "T1"];
const PLAN = [];
...
if (PLAN.length !== 100) throw new Error(`Plan hat ${PLAN.length} statt 100 Patterns`);
...
console.log(`Bank: ${BANK.split(/[\/]/).pop()} — ${belegt.length} Samples · Set 9 LANGSTRECKE (${BPM} BPM, ${PLAN.length} Patterns)`);
