/**
 * Erzeugt tekk3.all — tekk2.all (byte-genau, Anzeige 501–553) plus ein
 * frischer Schwung melodisches Material aus E:\ und G:\Mukke Stuff ab 554:
 * zwoelf neue Melos/Synths und vier neue Flaechen, ausgewaehlt per
 * Namens-/Dauer-Heuristik wie bei HARDTEKK — mit Sperrliste (Ohrproben-
 * Durchfaller) und Ausschluss aller Namen, die schon in der Bank liegen.
 *
 * Zweck (Nutzerwunsch 2026-08-16): genug Melo-Auswahl, damit Sets die
 * Besetzung pro Segment WECHSELN koennen statt eine Melo durchlaufen zu
 * lassen (Set 8 "WECHSELSPIEL" nutzt das).
 */
...
const QUELLE = "examples/e2s/tekk2.all";
const ZIEL = process.argv[2] ?? "examples/e2s/tekk3.all";
const WURZELN = ["E:\\", "G:\\Mukke Stuff"];

/** Ohrproben-Durchfaller (siehe make-hardtekk-bank.mjs) — bleiben draussen. */
const SPERRLISTE = [
...
const NEU = [
  { name: "Melo", kat: 16, anzahl: 12, dauer: [1000, 8000], muster: /melo|lead|synth|arp|chord|akkord|pluck|stab/i },
  { name: "Pad", kat: 14, anzahl: 4, dauer: [2000, 12000], muster: /pad|atmo|string|drone|choir|flaeche|fläche/i },
];
...
// ─── Bestaende scannen (wie make-hardtekk-bank) ─────────────────────────────
...
for (const k of NEU) {
  ...
  for (const p of alle) {
    ...
    if (vorhandeneNamen.has(kurz(p).toLowerCase())) continue; // schon in der Bank
