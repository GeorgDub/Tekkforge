/**
 * Erzeugt TEKK_SET10.e2sallpat — "DOPPELDECKER": 100 Patterns, 184 BPM,
 * fuer tekk4.all. Das harte Langformat: zehn Segmente in dunkler Harmonik
 * (phrygisch), Kick-lastige Figuren, und die Acid-/Vocal-Farben tragen die
 * Rotation. Struktur/Engine wie Set 9 (LANGSTRECKE), aber eigener Plan.
 */
...
const ZIEL = process.argv[2] ?? "examples/e2s/TEKK_SET10.e2sallpat";
const N = 64;
const BPM = Number(process.argv[4]) || 184;

const MONO1 = 0, MONO2 = 1, POLY2 = 3;

/** Phrygisch-dunkel — der Doppeldecker faehrt nachts. */
const THEMA = {
  A: {
  ...
  B: {
    akkorde: [[57, 60, 64], [58, 62, 65], [55, 59, 62], [52, 56, 59]], // Am Bb G E
    bass: [33, 34, 31, 28],
  },
};
...
const THEMEN_FOLGE = ["T4", "T5", "T3", "T6", "T5", "T4", "T2", "T5", "T6", "T3"];
const PLAN = [];
THEMEN_FOLGE.forEach((tk, seg) => {
  ...
  p("AUF 1", seg === 0 ? 0 : 2, "hart", 2);
  p("AUF 2", seg === 0 ? 1 : 3, "hart", 1);
  p("ANLAUF", 4, "roll", 1);
  p("DROP 1", 5, "hart", 2);
  p("DROP 2", 5, "hart", 1);
  p("DROP 3", 5, "hart", 2);
  p("KELLER", 4, "vier", 2);
  p("DRUCK", 5, "hart", 1);
  if (seg % 3 === 2) p("LUFT", -1, "kein", 1);
  else p("DROP 4", 5, "roll", 1);
  p("JAM", 2, "vier", 1, true);
});
...
console.log(`Bank: ${BANK.split(/[\/]/).pop()} — ${belegt.length} Samples · Set 10 DOPPELDECKER (${BPM} BPM, ${PLAN.length} Patterns)`);
